"""Testing AuthEvent objects of cbc_sdk.enterprise_edr"""
...
from cbc_sdk.enterprise_edr.auth_events import AuthEventQuery, AuthEventFacet
...
def test_auth_event_select_where(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    events_list = api.select(AuthEvent).where("auth_username:SYSTEM")
...
def test_auth_event_select_async(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    events_list = api.select(AuthEvent).where(event_id="DA9E269E-421D-469D-A212-9062888A02F4").execute_async()
...
def test_auth_event_select_by_id(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    auth_events = api.select(AuthEvent, "DA9E269E-421D-469D-A212-9062888A02F4")
...
def test_auth_event_select_details_async(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    # events_list = api.select(AuthEvent).where(process_pid=2000)
    events_list = api.select(AuthEvent).where(event_id="DA9E269E-421D-469D-A212-9062888A02F4")
...
def test_auth_event_details_only(cbcsdk_mock):
    """Testing AuthEvent with get_details - just the get_details REST API calls"""
    cbcsdk_mock.mock_request(
        "POST",
        "/api/investigate/v2/orgs/test/auth_events/detail_jobs",
        POST_AUTH_EVENT_SEARCH_JOB_RESP,
    ...
    api = cbcsdk_mock.api
    events = AuthEvent(api, initial_data={"event_id": "D06DC822-B25E-4162-A5A7-6166BFA9B8DF"})
...
def test_auth_event_select_compound(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    events_list = api.select(AuthEvent).where(process_pid=776).or_(parent_pid=608)
...
def test_auth_event_query_implementation(cbcsdk_mock):
    ...
    events_list = api.select(AuthEvent).where(f"event_id:{event_id}")
    assert isinstance(events_list, AuthEventQuery)
    assert events_list[0].event_id == event_id


def test_auth_event_timeout(cbcsdk_mock):
...
def test_auth_event_timeout_error(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    events_list = (api.select(AuthEvent).where("event_id:DA9E269E-421D-469D-A212-9062888A02F4").timeout(1))
    with pytest.raises(TimeoutError):
        list(events_list)
    events_list = (api.select(AuthEvent).where("event_id:DA9E269E-421D-469D-A212-9062888A02F4").timeout(1))
    with pytest.raises(TimeoutError):
...
def test_auth_event_query_sort(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
    events_list = (
        api.select(AuthEvent)
...
def test_auth_event_rows(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
    events_list = api.select(AuthEvent).where(process_pid=1000).set_rows(1500)
    assert events_list._batch_size == 1500
    with pytest.raises(ApiError) as ex:
        api.select(AuthEvent).where(process_pid=1000).set_rows("alabala")
    assert "Rows must be an integer." in str(ex)
    with pytest.raises(ApiError) as ex:
        api.select(AuthEvent).where(process_pid=1000).set_rows(10001)
    assert "Maximum allowed value for rows is 10000" in str(ex)


def test_auth_event_time_range(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
    events_list = (
        api.select(AuthEvent)
...
def test_auth_event_submit(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    events_list = api.select(AuthEvent).where(process_pid=1000)
    events_list._submit()
...
def test_auth_event_search(cbcsdk_mock):
    ...
    api = cbcsdk_mock.api
    events_list = api.select(AuthEvent).where(process_pid=828)
    events_list._search()
...
def test_auth_event_facet_select_where(cbcsdk_mock):
    """Testing AuthEvent Querying with select()"""
    cbcsdk_mock.mock_request(
        "POST",
...
def test_auth_event_facet_select_async(cbcsdk_mock):
    """Testing AuthEvent Querying with select()"""
    cbcsdk_mock.mock_request(
        "POST",
        "/api/investigate/v2/orgs/test/auth_events/facet_jobs",
...
def test_auth_event_facet_select_compound(cbcsdk_mock):
    """Testing AuthEvent Querying with select() and more complex criteria"""
    cbcsdk_mock.mock_request(
        "POST",
        "/api/investigate/v2/orgs/test/auth_events/facet_jobs",
...
def test_auth_event_facet_query_implementation(cbcsdk_mock):
    """Testing AuthEvent querying with where()."""
    cbcsdk_mock.mock_request(
        "POST",
        "/api/investigate/v2/orgs/test/auth_events/facet_jobs",
...
def test_auth_event_facet_timeout(cbcsdk_mock):
    """Testing AuthEventQuery.timeout()."""
    api = cbcsdk_mock.api
    query = (
        api.select(AuthEventFacet)
...
def test_auth_event_facet_query_add_range(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
...
def test_auth_event_facet_query_check_range(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
    range = ({"bucket_size": [], "start": "0D", "end": "20D", "field": "something"},)
    with pytest.raises(ApiError):
        api.select(AuthEventFacet).where(process_pid=1000).add_range(
            range
        ).add_facet_field("process_name")
...
def test_auth_event_facet_query_add_facet_field(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
    auth_events = (
        api.select(AuthEventFacet)
...
def test_auth_event_facet_query_add_facet_fields(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
    auth_events = (
        api.select(AuthEventFacet)
...
def test_auth_event_facet_query_add_facet_invalid_fields(cbcsdk_mock):
    """Testing AuthEvent results sort."""
    api = cbcsdk_mock.api
    with pytest.raises(TypeError):
        api.select(AuthEventFacet).where(process_pid=1000).add_facet_field(1337)


def test_auth_event_facet_limit(cbcsdk_mock):
    """Testing AuthEvent results limit."""
    api = cbcsdk_mock.api
    auth_events = (
        api.select(AuthEventFacet)
        .where(process_pid=1000)
...
def test_auth_event_facet_time_range(cbcsdk_mock):
    """Testing AuthEvent results range."""
    api = cbcsdk_mock.api
    auth_events = (
        api.select(AuthEventFacet)
        .where(process_pid=1000)
...
def test_auth_event_facet_submit(cbcsdk_mock):
    """Test _submit method of AuthEventQuery class"""
    cbcsdk_mock.mock_request(
        "POST",
        "/api/investigate/v2/orgs/test/auth_events/facet_jobs",
...
def test_auth_event_facet_count(cbcsdk_mock):
    """Test _submit method of AuthEventQuery class"""
    cbcsdk_mock.mock_request(
        "POST",
        "/api/investigate/v2/orgs/test/auth_events/facet_jobs",
...
def test_auth_event_search_async(cbcsdk_mock):
    """Test _search method of AuthEventQuery class"""
    cbcsdk_mock.mock_request(
        "POST",
        "/api/investigate/v2/orgs/test/auth_events/facet_jobs",
...
def test_auth_event_aggregation_wrong_field(cbcsdk_mock):
    ...
    with pytest.raises(ApiError):
        for i in (
            ...
            print(i)
    with pytest.raises(ApiError):
        for i in api.select(AuthEvent).where(process_pid=2000).group_results(1):
            print(i)
...
def test_auth_event_select_group_results(cbcsdk_mock):
    ...
    # invoke get_details() on the first AuthEvent in the list
