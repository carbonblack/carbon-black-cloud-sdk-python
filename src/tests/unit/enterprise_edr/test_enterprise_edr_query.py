...
@pytest.mark.parametrize('get_summary_response, get_process_search_response, guid', [
    (GET_PROCESS_SUMMARY_RESP, GET_PROCESS_SEARCH_JOB_RESULTS_RESP, "test-0002b226-000015bd-00000000-1d6225bbba74c00",),
    (GET_PROCESS_SUMMARY_RESP_1, GET_PROCESS_SEARCH_JOB_RESULTS_RESP_1, "test-00340b06-00000314-00000000-1d686b9e4d74f52",)
    ])
def test_query_count(cbcsdk_mock, get_summary_response, get_process_search_response, guid):
    ...
    cbcsdk_mock.mock_request("GET", "/api/investigate/v2/orgs/test/processes/search_jobs/2c292717-80ed-4f0d-845f-779e09470920/results", get_process_search_response)
...
def test_query_get_query_parameters(cbcsdk_mock, get_process_search_response, guid):
    ...
    assert process_query._get_query_parameters() == {"fields": [
            ...
            "process_reputation,ttp"], "process_guid": guid, "query": f'process_guid:{guid}'}
...
def test_query_validate_not_valid(cbcsdk_mock, get_process_search_response, guid):
    ...
    api = cbcsdk_mock.api
    # mock the GET of query parameter validation
    cbcsdk_mock.mock_request("GET", "/api/investigate/v1/orgs/test/processes/search_validation", GET_PROCESS_VALIDATION_RESP_INVALID)
    # mock the POST of a search
    cbcsdk_mock.mock_request("POST", "/api/investigate/v2/orgs/test/processes/search_job", POST_PROCESS_SEARCH_JOB_RESP)
    # mock the GET to check search status
    cbcsdk_mock.mock_request("GET", "/api/investigate/v1/orgs/test/processes/search_jobs/2c292717-80ed-4f0d-845f-779e09470920", GET_PROCESS_SEARCH_JOB_RESP)
...
def test_async_sort_by(cbcsdk_mock):
    ...
    assert async_query._sort == [{"field": "device_timestamp", "order": "ASC"}, {"field": "key_to_sort_by", "order": "DESC"}]
    assert async_query._default_args == {"sort": [{"field": "device_timestamp", "order": "ASC"}, {"field": "key_to_sort_by", "order": "DESC"}]}
    # update the sort direction for a field
    async_query.sort_by("key_to_sort_by", direction="ASC")
    assert async_query._sort == [{"field": "device_timestamp", "order": "ASC"}, {"field": "key_to_sort_by", "order": "ASC"}]
    assert async_query._default_args == {"sort": [{"field": "device_timestamp", "order": "ASC"}, {"field": "key_to_sort_by", "order": "ASC"}]}
