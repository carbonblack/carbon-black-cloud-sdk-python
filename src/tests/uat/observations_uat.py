...
from cbc_sdk.platform import Observation, NetworkThreatMetadata
...
SEARCH_VALIDATION = "{}api/investigate/v2/orgs/{}/observations/search_validation?{}"
...
def get_search_results():
    ...
    job_id = requests.post(
        START_SEARCH_JOB.format(HOSTNAME, ORG_KEY), headers=HEADERS, json=sdata
    ).json()["job_id"]
    time.sleep(0.5)
    results = requests.get(
        GET_SEARCH_RESULTS.format(HOSTNAME, ORG_KEY, job_id), headers=HEADERS
    ).json()
...
def get_details_results(observation_id):
    ...
    job_id = requests.post(
        START_DETAILS_JOB.format(HOSTNAME, ORG_KEY), headers=HEADERS, json=ddata
    ).json()["job_id"]
    time.sleep(0.5)
...
def get_seach_validation(q="*:*", mint="1641469642000", maxt="1678103242000"):
    """Get search suggestions"""
    query = f"q={q}&cb.min_backend_timestamp={mint}&cb.max_backend_timestamp={maxt}"
    results = requests.get(SEARCH_VALIDATION.format(HOSTNAME, ORG_KEY, query), headers=HEADERS)
    return results.json()["valid"]


def get_network_threat_metadata(rule_id):
    """Get network threat metadata"""
    results = requests.get(
        GET_NETWORK_THREAT_METADATA.format(HOSTNAME, ORG_KEY, rule_id), headers=HEADERS
    ).json()
...
def main():
    ...
    sdk_result = cb.observations_search_suggestions(query="device_id", count=1)
    assert (
        api_result == sdk_result
    ), f"Test Failed Expected: {api_result} Actual: {sdk_result}"
    print(TITLES[0] + "." * (SYMBOLS - len(TITLES[0]) - 2) + "OK")
    api_result = get_seach_validation()
    sdk_result = cb.observations_search_validation(
        query="*:*",
        min_backend_timestamp="1641469642000",
        max_backend_timestamp="1678103242000",
    )
