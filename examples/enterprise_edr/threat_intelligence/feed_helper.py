"""Advances the `begin_date` and `end_date` fields while polling the TAXII server...

...to iteratively get per-collection STIX content.
...
class FeedHelper():
    ...
    def __init__(self, start_date, size_of_request_in_minutes):
        """Initialize the FeedHelper object."""
        ...
        if isinstance(start_date, datetime):
            ...
            log.error("Start_date must be a string or datetime object. Received a start_time config value "
                      f"with unsupported type: {type(start_date)}")
            raise ValueError
        self.end_date = self.start_date + timedelta(minutes=self.size_of_request_in_minutes)
