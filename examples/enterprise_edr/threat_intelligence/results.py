...
class IOC_v2():
    ...
    def as_dict(self):
        """Return all attributes of this match type as a dict."""
        return {
            "id": str(self.id),
...
class AnalysisResult():
    ...
    def attach_ioc_v2(self, *, match_type=IOC_v2.MatchType.Equality, values, field, link):
        """Attach additional IOCs to the result."""
        self.iocs_v2.append(IOC_v2(analysis=self.id, match_type=match_type, values=values, field=field, link=link))

    def normalize(self):
        """Normalizes this result to make it palatable for the Enterprise EDR backend."""
