...
def platform():
    ...
    print(f"Using {BOLD}Platform Alert Search{UNBOLD} to find Egregor Ransomware Alerts, "
          f"based on Egregor Ransomware TTPs: \n{egregor_ttps}\n")
    ...
    ioc_alerts = platform_api.select(PlatformAlert).where(harmless_query).set_create_time(
        range="-6d").set_group_results(True)
...
def create_eedr_report(iocs, title="", description="", severity=10):
    ...
    return Report(enterprise_edr_api, from_watchlist=True, initial_data=reportdata)
...
def audit_remediation():
    ...
    audit_remediation_api.select(Result).run_id(chrome_ext_run.id)
    ...
    audit_remediation_api.select(Result).run_id(usb_devices_run.id)
